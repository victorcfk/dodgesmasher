...
public class Reporter : MonoBehaviour
{
    ...
    [SerializeField]
	public Vector2 ReporterUISize = new Vector2(64, 64);
    [SerializeField]
    private float _reporterMaxUISize = 100;
    [SerializeField]
    private float _reporterMinUISize = 16;
    [SerializeField]
    private float _maxMemSize = 20;
    [SerializeField]
    private int _numOfCirclesRequiredToBeDrawnToShow = 1;
	...
	public void Initialize()
	{
		...
		ReporterUISize.x = ReporterUISize.y = PlayerPrefs.GetFloat("Reporter_size", 64);
	...
	{
		int paddingX = (int)(ReporterUISize.x * 0.2f);
		int paddingY = (int)(ReporterUISize.y * 0.2f);
		...
		nonStyle.fontSize = (int)(ReporterUISize.y / 2);
		...
		lowerLeftFontStyle.fontSize = (int)(ReporterUISize.y / 2);
		...
		barStyle.fontSize = (int)(ReporterUISize.y / 2);
		...
		oddLogStyle.fontSize = (int)(ReporterUISize.y / 2);
		//oddLogStyle.wordWrap = true ;

		logButtonStyle = new GUIStyle();
		//logButtonStyle.wordWrap = true;
		logButtonStyle.fixedHeight = ReporterUISize.y;
		...
		//logButtonStyle.wordWrap = true;
		logButtonStyle.fontSize = (int)(ReporterUISize.y / 2);
		...
		selectedLogStyle.fixedHeight = ReporterUISize.y;
		...
		selectedLogStyle.fontSize = (int)(ReporterUISize.y / 2);

		selectedLogFontStyle = new GUIStyle();
		selectedLogFontStyle.normal.background = images.selectedImage;
		selectedLogFontStyle.fixedHeight = ReporterUISize.y;
		...
		//selectedLogStyle.wordWrap = true;
		selectedLogFontStyle.fontSize = (int)(ReporterUISize.y / 2);
		...
		stackLabelStyle.fontSize = (int)(ReporterUISize.y / 2);
		...
		logScrollerSkin.verticalScrollbar.fixedWidth = ReporterUISize.x * 2f;
		logScrollerSkin.horizontalScrollbar.fixedHeight = 0f;
		logScrollerSkin.verticalScrollbarThumb.fixedWidth = ReporterUISize.x * 2f;
		...
		graphScrollerSkin.horizontalScrollbar.fixedHeight = ReporterUISize.x * 2f;
		graphScrollerSkin.verticalScrollbarThumb.fixedWidth = 0f;
		graphScrollerSkin.horizontalScrollbarThumb.fixedHeight = ReporterUISize.x * 2f;
	...
	void calculateCurrentLog()
	{
		...
		if (selectedLog != null) {
			...
			if (newSelectedIndex == -1) {
				...
				if (newSelectedIndex != -1)
					scrollPosition.y = newSelectedIndex * ReporterUISize.y;
			}
			else {
				scrollPosition.y = newSelectedIndex * ReporterUISize.y;
			}
	...
	void DrawInfo()
	{
		...
		GUILayout.Space(ReporterUISize.x);
		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(buildFromContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(buildDate, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(systemInfoContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(deviceModel, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(deviceType, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(deviceName, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(graphicsInfoContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(SystemInfo.graphicsDeviceName, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(graphicsMemorySize, nonStyle, GUILayout.Height(ReporterUISize.y));
#if !UNITY_CHANGE1
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(maxTextureSize, nonStyle, GUILayout.Height(ReporterUISize.y));
#endif
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label("Screen Width " + Screen.width, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label("Screen Height " + Screen.height, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(showMemoryContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(systemMemorySize + " mb", nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label("Mem Usage Of Logs " + logsMemUsage.ToString("0.000") + " mb", nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		...
		GUILayout.Label("GC Memory " + gcTotalMemory.ToString("0.000") + " mb", nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(softwareContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(SystemInfo.operatingSystem, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(dateContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(System.DateTime.Now.ToString(), nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.Label(" - Application Started At " + logDate, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(showTimeContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(Time.realtimeSinceStartup.ToString("000"), nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(showFpsContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(fpsText, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(userContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(UserData, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Box(showSceneContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label(currentScene, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		GUILayout.Box(showSceneContent, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label("Unity Version = " + Application.unityVersion, nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label("Size = " + ReporterUISize.x.ToString("0.0"), nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		float _size = GUILayout.HorizontalSlider(ReporterUISize.x, _reporterMinUISize, _reporterMaxUISize, sliderBackStyle, sliderThumbStyle, GUILayout.Width(Screen.width * 0.5f));
		if (ReporterUISize.x != _size) {
			ReporterUISize.x = ReporterUISize.y = _size;
			initializeStyle();
		}
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		if (GUILayout.Button(backContent, barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
	...
	void drawInfo_enableDisableToolBarButtons()
	{
		GUILayout.BeginHorizontal();
		GUILayout.Space(ReporterUISize.x);
		GUILayout.Label("Hide or Show tool bar buttons", nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.Space(ReporterUISize.x);
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		if (GUILayout.Button(clearOnNewSceneContent, (showClearOnNewSceneLoadedButton) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showClearOnNewSceneLoadedButton = !showClearOnNewSceneLoadedButton;
		}

		if (GUILayout.Button(showTimeContent, (showTimeButton) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
		...
		if (GUILayout.Button(showSceneContent, (showSceneButton) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showSceneButton = !showSceneButton;
		}
		tempRect = GUILayoutUtility.GetLastRect();
		GUI.Label(tempRect, currentScene, lowerLeftFontStyle);
		if (GUILayout.Button(showMemoryContent, (showMemButton) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showMemButton = !showMemButton;
		}
		tempRect = GUILayoutUtility.GetLastRect();
		GUI.Label(tempRect, gcTotalMemory.ToString("0.0"), lowerLeftFontStyle);

		if (GUILayout.Button(showFpsContent, (showFpsButton) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
		...
		if (GUILayout.Button(searchContent, (showSearchText) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
	...
	void DrawReport()
	{
		...
		GUILayout.FlexibleSpace();*/
		GUILayout.Label("Select Photo", nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Label("Coming Soon", nonStyle, GUILayout.Height(ReporterUISize.y));
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.FlexibleSpace();
		if (GUILayout.Button(backContent, barStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y))) {
			currentView = ReportView.Logs;
		}
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
	...
	void drawToolBar()
	{
		...
		toolBarRect.height = ReporterUISize.y * 2f;
		...
		GUI.skin = toolbarScrollerSkin;
		Vector2 drag = getDrag();
		if ((drag.x != 0) && (downPos != Vector2.zero) && (downPos.y > Screen.height - ReporterUISize.y * 2f)) {
		...
		if (GUILayout.Button(clearContent, barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			clear();
		}
		if (GUILayout.Button(collapseContent, (collapse) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			collapse = !collapse;
			calculateCurrentLog();
		}
		if (showClearOnNewSceneLoadedButton && GUILayout.Button(clearOnNewSceneContent, (clearOnNewSceneLoaded) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			clearOnNewSceneLoaded = !clearOnNewSceneLoaded;
		}

		if (showTimeButton && GUILayout.Button(showTimeContent, (showTime) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showTime = !showTime;
		}
		if (showSceneButton) {
			tempRect = GUILayoutUtility.GetLastRect();
			GUI.Label(tempRect, Time.realtimeSinceStartup.ToString("0.0"), lowerLeftFontStyle);
			if (GUILayout.Button(showSceneContent, (showScene) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
				showScene = !showScene;
			}
			tempRect = GUILayoutUtility.GetLastRect();
			GUI.Label(tempRect, currentScene, lowerLeftFontStyle);
		...
		if (showMemButton) {
			if (GUILayout.Button(showMemoryContent, (showMemory) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
		...
		if (showFpsButton) {
			if (GUILayout.Button(showFpsContent, (showFps) ? buttonActiveStyle : barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
				showFps = !showFps;
			}
			tempRect = GUILayoutUtility.GetLastRect();
			GUI.Label(tempRect, fpsText, lowerLeftFontStyle);
		...
		if (showSearchText) {
			GUILayout.Box(searchContent, barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2));
			tempRect = GUILayoutUtility.GetLastRect();
		...
		if (GUILayout.Button(infoContent, barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
		...
		if (GUILayout.Button(logContent, nonStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showLog = !showLog;
			calculateCurrentLog();
		}
		if (GUILayout.Button(logsText, nonStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showLog = !showLog;
			calculateCurrentLog();
		}
		GUILayout.EndHorizontal();
		GUILayout.BeginHorizontal((showWarning) ? buttonActiveStyle : barStyle);
		if (GUILayout.Button(warningContent, nonStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showWarning = !showWarning;
			calculateCurrentLog();
		}
		if (GUILayout.Button(logsWarningText, nonStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showWarning = !showWarning;
			calculateCurrentLog();
		}
		GUILayout.EndHorizontal();
		GUILayout.BeginHorizontal((showError) ? buttonActiveStyle : nonStyle);
		if (GUILayout.Button(errorContent, nonStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showError = !showError;
			calculateCurrentLog();
		}
		if (GUILayout.Button(logsErrorText, nonStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			showError = !showError;
			calculateCurrentLog();
		}
		GUILayout.EndHorizontal();

		if (GUILayout.Button(closeContent, barStyle, GUILayout.Width(ReporterUISize.x * 2), GUILayout.Height(ReporterUISize.y * 2))) {
			show = false;
	...
	void DrawLogs()
	{
		...
		int totalVisibleCount = (int)(Screen.height * 0.75f / ReporterUISize.y);
		...
		int index = 0;
		int beforeHeight = (int)(startIndex * ReporterUISize.y);
		...
		for (int i = startIndex; (startIndex + index) < endIndex; i++) {
			...
			countRect.y = ReporterUISize.y * i;
			if (beforeHeight > 0)
				countRect.y += 8;//i will check later why
			countRect.width = w;
			countRect.height = ReporterUISize.y;

			if (scrollerVisible)
				countRect.x -= ReporterUISize.x * 2;

			Sample sample = samples[log.sampleId];
			fpsRect = countRect;
			if (showFps) {
				tempContent.text = sample.fpsText;
				w = currentLogStyle.CalcSize(tempContent).x + ReporterUISize.x;
				fpsRect.x -= w;
				fpsRect.width = ReporterUISize.x;
				fpsLabelRect = fpsRect;
				fpsLabelRect.x += ReporterUISize.x;
				fpsLabelRect.width = w - ReporterUISize.x;
			}
			...
			if (showMemory) {
				...
				w = currentLogStyle.CalcSize(tempContent).x + ReporterUISize.x;
				memoryRect.x -= w;
				memoryRect.width = ReporterUISize.x;
				memoryLabelRect = memoryRect;
				memoryLabelRect.x += ReporterUISize.x;
				memoryLabelRect.width = w - ReporterUISize.x;
			}
			...
			if (showScene) {
				...
				w = currentLogStyle.CalcSize(tempContent).x + ReporterUISize.x;
				sceneRect.x -= w;
				sceneRect.width = ReporterUISize.x;
				sceneLabelRect = sceneRect;
				sceneLabelRect.x += ReporterUISize.x;
				sceneLabelRect.width = w - ReporterUISize.x;
			}
			timeRect = sceneRect;
			if (showTime) {
				...
				w = currentLogStyle.CalcSize(tempContent).x + ReporterUISize.x;
				timeRect.x -= w;
				timeRect.width = ReporterUISize.x;
				timeLabelRect = timeRect;
				timeLabelRect.x += ReporterUISize.x;
				timeLabelRect.width = w - ReporterUISize.x;
			}
			...
			if (log == selectedLog) {
				GUILayout.Box(content, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y));
				...
				if (GUILayout.Button(content, nonStyle, GUILayout.Width(ReporterUISize.x), GUILayout.Height(ReporterUISize.y))) {
		...
		int afterHeight = (int)((totalCount - (startIndex + totalVisibleCount)) * ReporterUISize.y);
		if (afterHeight > 0) {
		...
		buttomRect.x = 0f;
		buttomRect.y = Screen.height - ReporterUISize.y;
		buttomRect.width = Screen.width;
		buttomRect.height = ReporterUISize.y;
	...
	void drawGraph()
	{
		...
		graphMaxRect.height = ReporterUISize.y;
		...
		graphMinRect.y = stackRect.y + stackRect.height - ReporterUISize.y;
		graphMinRect.height = ReporterUISize.y;
	...
	void drawStack()
	{
		...
		if (selectedLog != null) {
			...
			GUILayout.EndHorizontal();
			GUILayout.Space(ReporterUISize.y * 0.25f);
			GUILayout.BeginHorizontal();
			GUILayout.Label(selectedLog.stacktrace, stackLabelStyle);
			GUILayout.EndHorizontal();
			GUILayout.Space(ReporterUISize.y);
			GUILayout.EndScrollView();
			GUILayout.EndArea();
	...
	public void OnGUIDraw()
	{
		...
		logsRect.x = 0f;
		logsRect.y = ReporterUISize.y * 2f;
		logsRect.width = Screen.width;
		logsRect.height = Screen.height * 0.75f - ReporterUISize.y * 2f;
		...
		stackRect.height = Screen.height * 0.25f - ReporterUISize.y;



		detailRect.x = 0f;
		detailRect.y = Screen.height - ReporterUISize.y * 3;
		detailRect.width = Screen.width;
		detailRect.height = ReporterUISize.y * 3;
	...
	bool isGestureDone()
	{
		...
		if (gestureLength > gestureBase && gestureSum.magnitude < gestureBase / 2) {
			gestureDetector.Clear();
			gestureCount++;
			if (gestureCount >= _numOfCirclesRequiredToBeDrawnToShow)
				return true;
		}
	...
	{
		startIndex = (int)(scrollPosition.y / ReporterUISize.y);
	...
	void AddLog(string condition, string stacktrace, LogType type)
	{
		...
		if (TotalMemUsage > _maxMemSize) {
			clear();
			Debug.Log("Memory Usage Reach" + _maxMemSize + " mb So It is Cleared");
			return;
		}
		...
		if (newLogAdded) {
			...
			if (startIndex >= (totalCount - totalVisibleCount))
				scrollPosition.y += ReporterUISize.y;
		}
	...
	//new scene is loaded. It was originally onlevelwasloaded which will be deprecated
	void OnLevelLoad(Scene s, LoadSceneMode mode)
	{
	...
	void OnApplicationQuit()
	{
		...
		PlayerPrefs.SetFloat("Reporter_size", ReporterUISize.x);
