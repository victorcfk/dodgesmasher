...
namespace CodeStage.Maintainer
{
	public class SearchResultsStorage
	{
		...
		private static UsagesRecord[] usagesSearchResults;
		...
		public static UsagesRecord[] UsagesSearchResults
		{
			get
			{
				if (cleanerSearchResults == null)
				...
				return usagesSearchResults;
			}
			set
			{
				usagesSearchResults = value;
				Save();
			}
		}
		...
		public static void Load()
		{
			if (File.Exists(PATH))
			{
				...
				try
				{
					...
					usagesSearchResults = bf.Deserialize(stream) as UsagesRecord[];
				}
				catch (Exception ex)
				{
			...
			else
			{
				...
				usagesSearchResults = new UsagesRecord[0];
			}
		}
		...
		public static void Save()
		{
			...
			bf.Serialize(stream, usagesSearchResults);
			stream.Close();
		}
	}
}
