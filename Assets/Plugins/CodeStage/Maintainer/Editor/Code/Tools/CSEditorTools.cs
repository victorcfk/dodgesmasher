...
namespace CodeStage.Maintainer.Tools
{
	public class CSEditorTools
	{
		...
		public static ActiveEditorTracker GetActiveEditorTrackerForSelectedObject()
		{
			...
			{
				Debug.LogError(Maintainer.LOG_PREFIX + "Can't find UnityEditor.InspectorWindow type!\nPlease report to " + Maintainer.SUPPORT_EMAIL);
				return null;
			}
			...
			{
				Debug.LogError(Maintainer.LOG_PREFIX + "Can't get an InspectorWindow!\nPlease report to " + Maintainer.SUPPORT_EMAIL);
				return null;
			}

#if UNITY_5_5_OR_NEWER
			PropertyInfo trackerProperty = inspectorWindowType.GetProperty("tracker");
			if (trackerProperty == null)
			{
				return GetActiveTrackerUsingMethod(inspectorWindowType, inspectorWindow);
			}

			return (ActiveEditorTracker)trackerProperty.GetValue(inspectorWindow, null);
#else
			return GetActiveTrackerUsingMethod(inspectorWindowType, inspectorWindow);
#endif
		}
		...
		{
			MethodInfo getTrackerMethod = inspectorWindowType.GetMethod("GetTracker");

			if (getTrackerMethod == null)
			{
				Debug.LogError(Maintainer.LOG_PREFIX + "Can't find an InspectorWindow.GetTracker() method!\nPlease report to " + Maintainer.SUPPORT_EMAIL);
				return null;
			}
		...
		public static string[] GetProjectSelections(bool includeFolders)
		{
			...
			List<string> paths = new List<string>(selectedGUIDs.Length);
		...
		public static ProjectAsset[] GetProjectSelectedObjects(bool includeFolders)
		{
			Object[] selectedObjects = Selection.objects;
			var filteredSelectedObjects = new List<ProjectAsset>(selectedObjects.Length);
			...
			foreach (Object unityObject in selectedObjects)
			{
				...
				if (!includeFolders && Directory.Exists(path)) continue;

				filteredSelectedObjects.Add(new ProjectAsset(path, unityObject));
			}

			return filteredSelectedObjects.ToArray();
		}
	}
}
