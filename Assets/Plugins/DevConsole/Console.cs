...
namespace DevConsole{
	...
	public class Console : MonoBehaviour,ISerializationCallbackReceiver {
			...
			if (showHelp && helpEnabled && inputText.Trim ()!=string.Empty){
				...
				if (candidates.Count != 0){
                    ...
                    for (int i = 0; i < candidates.Count; i++) {
                        string signature = candidates[selectedCandidate] == candidates[i] ? "<color=yellow>" + candidates[i].signature.raw+
                            "</color>" : candidates[i].signature.raw;
		...
		void BasePrintOnGUI(string text, string color){
			text = "> " + text;
			int numLineJumps = 1;
			string time = (extraSettings.showTimeStamp?"["+System.DateTime.Now.ToShortTimeString()+"]  ":string.Empty);
		...
		void AddText(string text, string color){
			consoleText.Append(string.Format("{0}<color=#{1}>{2}</color>",extraSettings.showTimeStamp?"["+System.DateTime.Now.ToShortTimeString()+"]  ":string.Empty, color, text));
		}
		...
		void ExecuteCommandInternal(string command){
            CommandExecuter executer = manager.GetCommandExecuter(command);
        ...
        static void HelpCommand() {
            ...
            for (int i = 0; i < commands.Length; i++) {
                text.Append(commands[i].name + (commands[i].description == null ? string.Empty : " - " + commands[i].description));
