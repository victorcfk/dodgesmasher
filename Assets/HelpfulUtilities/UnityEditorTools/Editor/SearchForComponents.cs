...
public class SearchForComponents : EditorWindow {
	[MenuItem("Tools/Search For Components In Prefabs")]
	static void Init () {
	...
	int boldPtr = -1;
	GUIStyle lastSelectedPrefabLabel;
	void OnGUI () {

		lastSelectedPrefabLabel = new GUIStyle(GUI.skin.label);
		lastSelectedPrefabLabel.fontStyle = FontStyle.Bold;
		lastSelectedPrefabLabel.normal.textColor = Color.white;
			...
			if ( listResult != null ) {
				if ( listResult.Count == 0 ) {
					...
					for (int i = 0; i < listResult.Count; i++) {
						...
						string s = listResult [i];

						GUILayout.BeginHorizontal ();

						if(i == boldPtr)
						{
							GUILayout.Label (s, lastSelectedPrefabLabel, GUILayout.Width (4 * position.width / 5));
						}
						else
						{
							GUILayout.Label (s, GUILayout.Width (4 * position.width / 5));
						}

						if (GUILayout.Button ("Select", GUILayout.Width (position.width / 5 - 10))) {
	...
	void DisplayResults ( ref Vector2 scroller, ref List<ComponentNames> list ) {
		...
		for (int i = 0; i < list.Count; i++) {
			...
			if(i == boldPtr)
			{
				GUILayout.Label (c.assetPath, lastSelectedPrefabLabel, GUILayout.Width (position.width / 5 * 4));
			}
			else
			{
				GUILayout.Label (c.assetPath, GUILayout.Width (position.width / 5 * 4));
